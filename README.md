...
<details>
<summary><b>保存了 API Key，提问却报「API Key 无效或已过期(401)」</b></summary>

三个地方按顺序排查：

**① Key 输入框是脱敏的，而「留空」的语义是「不修改」。**
所以换 Key 时必须**先把输入框清空**再粘贴完整 Key —— 只是点进去看看就点保存，
等于什么都没改。保存成功后按钮上的 `●` 未保存标记会消失，
没改动时也会提示「没有需要更新的字段」。

**② 保存成功后会自动验证一次 Key。**
后端对 Key 只做格式校验，**保存成功不等于能用** —— 过期、被吊销、余额不足
都要真实调一次才知道。所以保存后会自动调一次模型，结果直接显示在
「测试连接」的结果框里。看到绿色的 `✓ 连接成功` 才算真的配好了。

**③ 右上角的「服务正常」绿灯不代表 Key 可用。**
就绪探针只检查「配没配」，不发真实请求（它会被前端每 30 秒轮询一次，
每次都调模型既慢又费钱）。想确认 Key 能不能用，只能走「测试连接」。

> 备选方案：也可以直接编辑 `data/runtime_settings.json` 里的 `llm_api_key`
> 再重启服务。但改完记得用「测试连接」验一次。
