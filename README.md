...
| P3 | 分块可视化与调参 + 对话历史持久化 | ✅ 已完成 |
| P4 | 工程化加固（异步任务队列/限流/JWT） | ⏳ |
| P5 | 评测体系与量化对比实验 | ⏳ |
| P6 | Vue3 完整前端（替换单页控制台） | ⏳ |
...
| 策略 | 父块 | 子块 | 平均长度 | 章节覆盖率 |
|---|---|---|---|---|
| `parent_child`（默认） | 7 | 11 | 157.7 | **100%** |
| `recursive` | 2 | 17 | 96.4 | 0% |
| `fixed`（基线） | 2 | 7 | 286.6 | 0% |

> 所有参数都可以在 Web 界面上实时调参并预览结果，不用重新上传文档。

### 会话接口

| 方法 | 路径 | 说明 |
|---|---|---|
| GET/POST | `/api/v1/conversations` | 会话列表 / 新建 |
| GET/PATCH/DELETE | `/api/v1/conversations/{id}` | 详情（含消息）/ 重命名 / 软删 |
| DELETE | `/api/v1/conversations/{id}/messages` | 清空消息（保留会话） |
| GET | `/api/v1/conversations/stats` | 会话数 / 消息数 / 拒答率 |
